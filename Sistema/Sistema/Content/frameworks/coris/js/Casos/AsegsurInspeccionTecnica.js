...
$("#tbSolicitadoPor").typeahead({ source: function (query, process) { return $.ajax({ type: "POST", url: "../WebService/WSAjustes.asmx/BuscarAsegsurUsuario", data: "{'buscar': '" + query + "'}", contentType: "application/json; charset=utf-8", dataType: "json", success: function (jsonData) { arrayNames = []; arrayIds = {}; $("#tbSolicitadoPorCod").val('-1'); var jsonArray = jQuery.parseJSON(jsonData.d); $.each(jsonArray, function (key, value) { arrayIds[value.nombres] = value; arrayNames.push(value.nombres); }); return process(arrayNames); }, error: AjaxError }); }, updater: function (item) { $("#tbSolicitadoPorCod").val(arrayIds[item].cod_auto); return item; } });
