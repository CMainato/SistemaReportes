...
function CargarTipoVehiculo(asis_cod) {

    var select = $('#ddlMarcaVehiculo');
    select.find('option').remove().end().append('<option value="-1">SELECCIONAR</option>').val('-1');

    if (asis_cod != '-1') {
        $.ajax({
            type: "POST",
            url: "../WebService/WSComun.asmx/CargaTipoVehiculo",
            contentType: "application/json; charset=utf-8",
            data: "{'opcion': '" + asis_cod + "'} ",
            dataType: "json",
            success: function (jsonData) {
                ...
                $.each(jsonArray, function (key, value) {
                    select.append('<option value="' + value.cod_marca + '">' + value.desc_marca + '</option>').val(value.cod_marca);
                });
